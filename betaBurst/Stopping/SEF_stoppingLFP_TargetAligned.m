...
% Extracted by:
targetBeta.timing.canceled = SEF_stoppingLFP_getAverageBurstTimeTarget...
    (corticalLFPcontacts.all,executiveBeh.ttx_canc, bayesianSSRT, sessionLFPmap);
targetBeta.timing.noncanceled = SEF_stoppingLFP_getAverageBurstTimeTarget...
    (corticalLFPcontacts.all,executiveBeh.ttx.sNC, bayesianSSRT, sessionLFPmap);
targetBeta.timing.nostop = SEF_stoppingLFP_getAverageBurstTimeTarget...
...
for session = 1:29
    ...
    for trlIdx = 1:3
        ...
        for varIdx = 1:length(varList)
            ...
            targetBeta.sessionTiming.(trialType).(varType)(session,1) = nanmean(targetBeta.timing.(trialType).(varType)(sessionLFPidx)); 
        end
    end
end
    
targetBeta.sessionTiming.canceled = struct2table(targetBeta.sessionTiming.canceled);
targetBeta.sessionTiming.noncanceled = struct2table(targetBeta.sessionTiming.noncanceled);
targetBeta.sessionTiming.nostop = struct2table(targetBeta.sessionTiming.nostop);

%% Find out beta-burst parameters/functional properties

canceledBurstTime = targetBeta.timing.canceled.mean_burstTime;
canceledBurstFreq = targetBeta.timing.canceled.mean_burstFreq;
canceledBurstOnset = targetBeta.timing.canceled.mean_burstOnset;
canceledBurstOffset = targetBeta.timing.canceled.mean_burstOffset;
canceledBurstDuration = targetBeta.timing.canceled.mean_burstDuration;
canceledBurstVolume = targetBeta.timing.canceled.mean_burstVolume;

noncanceledBurstTime = targetBeta.timing.noncanceled.mean_burstTime;
noncanceledBurstFreq = targetBeta.timing.noncanceled.mean_burstFreq;
noncanceledBurstOnset = targetBeta.timing.noncanceled.mean_burstOnset;
noncanceledBurstOffset = targetBeta.timing.noncanceled.mean_burstOffset;
noncanceledBurstDuration = targetBeta.timing.noncanceled.mean_burstDuration;
noncanceledBurstVolume = targetBeta.timing.noncanceled.mean_burstVolume;

nostopBurstTime = targetBeta.timing.nostop.mean_burstTime;
nostopBurstFreq = targetBeta.timing.nostop.mean_burstFreq;
nostopBurstOnset = targetBeta.timing.nostop.mean_burstOnset;
nostopBurstOffset = targetBeta.timing.nostop.mean_burstOffset;
nostopBurstDuration = targetBeta.timing.nostop.mean_burstDuration;
nostopBurstVolume = targetBeta.timing.nostop.mean_burstVolume;
...
writetable(meanBurstTimeTable,...
    'D:\projectCode\project_stoppingLFP\data\exportJASP\LFP_meanburstTime_target.csv','WriteRowNames',true)
...
% % % % burstParameters_stoppingBeh(1,1)=gramm('x',targetBeta.sessionTiming.nostop.mean_burstTime(sessions),'y',targetBeta.sessionTiming.nostop.mean_ssrt(sessions));
% % % % burstParameters_stoppingBeh(1,2)=gramm('x',targetBeta.sessionTiming.noncanceled.mean_burstTime(sessions),'y',targetBeta.sessionTiming.noncanceled.mean_ssrt(sessions));
% % % % burstParameters_stoppingBeh(1,3)=gramm('x',targetBeta.sessionTiming.canceled.mean_burstTime(sessions),'y',targetBeta.sessionTiming.canceled.mean_ssrt(sessions));
% % % % %   STD burst time and SSRT relationship
% % % % burstParameters_stoppingBeh(2,1)=gramm('x',targetBeta.sessionTiming.nostop.std_burstTime(sessions),'y',targetBeta.sessionTiming.nostop.std_ssrt(sessions));
% % % % burstParameters_stoppingBeh(2,2)=gramm('x',targetBeta.sessionTiming.noncanceled.std_burstTime(sessions),'y',targetBeta.sessionTiming.noncanceled.std_ssrt(sessions));
% % % % burstParameters_stoppingBeh(2,3)=gramm('x',targetBeta.sessionTiming.canceled.std_burstTime(sessions),'y',targetBeta.sessionTiming.canceled.std_ssrt(sessions));
% % % % 
% % % % %   p(Burst) and trigger failures
% % % % burstParameters_stoppingBeh(3,1)=gramm('x',targetBeta.sessionTiming.nostop.pTrials_burst(sessions),'y',targetBeta.sessionTiming.nostop.triggerFailures(sessions));
% % % % burstParameters_stoppingBeh(3,2)=gramm('x',targetBeta.sessionTiming.noncanceled.pTrials_burst(sessions),'y',targetBeta.sessionTiming.noncanceled.triggerFailures(sessions));
% % % % burstParameters_stoppingBeh(3,3)=gramm('x',targetBeta.sessionTiming.canceled.pTrials_burst(sessions),'y',targetBeta.sessionTiming.canceled.triggerFailures(sessions));
% % % % 
% % % % alphaLevel = 0.7;
