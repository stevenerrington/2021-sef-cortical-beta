...
for ii = 1:length(laminarContacts.all)
    ...
    for ssdIdx = 1:length(executiveBeh.inh_SSD{session})
        ...
        ctrials = []; ctrials = executiveBeh.ttm_c.C{session,ssdIdx}.all;
        nstrials = []; nstrials = executiveBeh.ttm_c.GO_C{session,ssdIdx}.all;
        % Fixation aligned BBDF
        c_temp_fix(ssdIdx,:) = nanmean(bbdfData.bbdf.target(ctrials, :));
        ns_temp_fix(ssdIdx,:) = nanmean(bbdfData.bbdf.target(nstrials, :));
        diff_temp_fix(ssdIdx,:) = c_temp_fix(ssdIdx,:)-ns_temp_fix(ssdIdx,:);
        
        % SSD aligned BBDF
        c_temp_ssd(ssdIdx,:) = nanmean(bbdfData.bbdf.ssd(ctrials, :));
        ns_temp_ssd(ssdIdx,:) = nanmean(bbdfData.bbdf.ssd(nstrials, :));
        diff_temp_ssd(ssdIdx,:) = c_temp_ssd(ssdIdx,:)-ns_temp_ssd(ssdIdx,:);
        ...
        diff_temp_sacc(ssdIdx,:) = nc_temp_sacc(ssdIdx,:)-ns_temp_sacc(ssdIdx,:);
        
        % Tone aligned BBDF
        c_temp_tone(ssdIdx,:) = nanmean(bbdfData.bbdf.tone(ctrials, :));
        ns_temp_tone(ssdIdx,:) = nanmean(bbdfData.bbdf.tone(nstrials, :));
        diff_temp_tone(ssdIdx,:) = c_temp_tone(ssdIdx,:)-ns_temp_tone(ssdIdx,:);
    end
...
%     bbdf_depth_diffAll_Fix(depth,:,session-13) = nanmean(diff_temp_fix);
%     bbdf_depth_diffAll_SSD(depth,:,session-13) = nanmean(diff_temp_ssd);
%     bbdf_depth_diffAll_Saccade(depth,:,session-13) = nanmean(diff_temp_sacc);
%     bbdf_depth_diffAll_tone(depth,:,session-13) = nanmean(diff_temp_tone);    
%     
    bbdf_depth_diffAll_Fix(depth,:,session-13) = nanmean(c_temp_fix) - nanmean(ns_temp_fix);
    bbdf_depth_diffAll_SSD(depth,:,session-13) = nanmean(c_temp_ssd) - nanmean(ns_temp_ssd);
    bbdf_depth_diffAll_Saccade(depth,:,session-13) = nanmean(nc_temp_sacc) - nanmean(ns_temp_sacc);
    bbdf_depth_diffAll_tone(depth,:,session-13) = nanmean(c_temp_tone) - nanmean(ns_temp_tone);        
...
for session = 1:16
    ...
    % ...for fixation
    bbdf_depth_norm_Fix(:,:,session) = (bbdf_depth_diffAll_Fix(:,plotWindow_fix,session)-...
        nanmean(nanmean(bbdf_depth_diffAll_Fix(:,plotWindow_fix,session))))./...
        nanstd(nanstd(bbdf_depth_diffAll_Fix(:,plotWindow_fix,session)));
    
    % ...for SSD
    bbdf_depth_norm_SSD(:,:,session) = (bbdf_depth_diffAll_SSD(:,plotWindow_ssd,session)-...
        nanmean(nanmean(bbdf_depth_diffAll_SSD(:,plotWindow_ssd,session))))./...
        nanstd(nanstd(bbdf_depth_diffAll_SSD(:,plotWindow_ssd,session)));
    
    % ...for saccade
    bbdf_depth_norm_Saccade(:,:,session) = (bbdf_depth_diffAll_Saccade(:,plotWindow_saccade,session)-...
        nanmean(nanmean(bbdf_depth_diffAll_Saccade(:,plotWindow_saccade,session))))./...
        nanstd(nanstd(bbdf_depth_diffAll_Saccade(:,plotWindow_saccade,session)));
    
    % ...for saccade
    bbdf_depth_norm_Tone(:,:,session) = (bbdf_depth_diffAll_tone(:,plotWindow_tone,session)-...
        nanmean(nanmean(bbdf_depth_diffAll_tone(:,plotWindow_tone,session))))./...
        nanstd(nanstd(bbdf_depth_diffAll_tone(:,plotWindow_tone,session)));
end
